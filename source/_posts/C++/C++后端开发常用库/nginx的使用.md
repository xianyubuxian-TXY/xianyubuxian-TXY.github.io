...
### （1）Nginx的反向代理
- 客户端对目标服务器是无感知的，客户端将请求发送到反向代理服务器，反向代理服务器再将请求发送到目标服务器，获取响应数据后返回给客户端 ——> 只暴露了反向代理服务器的IP地址，而隐藏了真实服务器的IP地址
![](https://cdn.jsdelivr.net/gh/xianyubuxian-TXY/hexo-images/images/20260308210527953.png)

### （2）Nginx的负载均衡
- 在“Nginx的反向代理”我们知道：客户端请求先发送到反向代理服务器，在由反向代理服务器发送到目标服务器
- 而负载均衡就是让反向代理服务器通过指定的负载均衡策略，将请求合理的分发到不同目标服务器上，避免某个目标服务器接收过多请求而导致超载，而其它服务器却因为请求太少而利用率太低
```python
┌─────────────────────────────────────────────────────────────────────────┐
│                      负载均衡（反向代理的升级版）                         │
│                                                                         │
...
## 3.Nginx的反向代理 + 负载均衡 工作流程
### （1）工作流程
```bash
1. Master 进程：读取配置 → bind+listen → fork Worker 进程（数量=auto/配置）

2. Worker 进程：继承 listen_fd → 注册到 epoll → 进入单线程事件循环

3. 客户端新连接到来：Worker 抢占 → accept → 新 fd 注册到 epoll

4. 连接事件发生：epoll_wait 返回 → 同一线程用非阻塞I/O处理 
   ├─ 读取客户端请求
   ├─ 根据负载均衡策略选择后端
   ├─ 非阻塞连接后端 + 转发请求 （可以：连接池复用）
   ├─ 非阻塞读取后端响应
   └─ 非阻塞返回给客户端

5. 关键点：Worker全程单线程 + 非阻塞 I/O + 事件驱动，无需线程池！
```
...
### （2）连接池复用（keepalive）
![](https://cdn.jsdelivr.net/gh/xianyubuxian-TXY/hexo-images/images/20260308214526431.png)![](https://cdn.jsdelivr.net/gh/xianyubuxian-TXY/hexo-images/images/20260308214553989.png)![](https://cdn.jsdelivr.net/gh/xianyubuxian-TXY/hexo-images/images/20260308214627045.png)
