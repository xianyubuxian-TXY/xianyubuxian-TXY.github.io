...
│   produce()          poll()              flush()                               │
│      │                 │                    │                                  │
│      │ "把消息放进去"   │ "处理回调结果"      │ "等所有消息发完"                  │
│      ▼                 ▼                    ▼                                  │
├────────────────────────────────────────────────────────────────────────────────┤
│                         librdkafka 内部 (后台线程)                              │
│                                                                                │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐     │
│  │ 发送队列     │ →  │ 批量/压缩    │ →  │ 网络发送    │ →  │ 结果队列     │     │
│  │ (outq)      │    │             │    │ → Broker    │    │ (dr_queue)  │     │
│  └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘     │
│        ↑                                                        │              │
│        │                                                        │              │
│   produce()                                                  poll()            │
...
└────────────────────────────────────────────────────────────────────────────────┘
```
![](https://cdn.jsdelivr.net/gh/xianyubuxian-TXY/hexo-images/images/20260206144503782.png)

#### <1>produce — 放入消息
```cpp
RdKafka::ErrorCode produce(
    const std::string &topic,      // 目标 topic
    int32_t partition,             // 分区（-1 = 自动）
    int msgflags,                  // 消息标志
    void *payload,                 // 消息内容
    size_t len,                    // 消息长度
    const void *key,               // 消息 key
    size_t key_len,                // key 长度
    int64_t timestamp,             // 时间戳
    void *msg_opaque               // 用户自定义数据
);
```
##### 1）功能
- 将消息放入**内部发送队列**
...
│   │               参数校验                                    │   │
│   │  - topic 是否存在？                                       │   │
│   │  - 消息大小是否超限？(message.max.bytes)                  │   │
│   └─────────────────────────────────────────────────────────┘   │
│      │                                                          │
│      ▼                                                          │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │               分区选择                                    │   │
│   │  - 如果指定了 key → 根据 key 哈希选择分区                  │   │
│   │  - 如果没有 key → 轮询或随机选择分区                       │   │
│   └─────────────────────────────────────────────────────────┘   │
│      │                                                          │
│      ▼                                                          │
...
│   │                                                         │   │
│   │  ┌───┬───┬───┬───┬───┬───┬───┬───┐                      │   │
│   │  │msg│msg│msg│msg│msg│msg│msg│NEW│ ← 新消息              │   │
│   │  └───┴───┴───┴───┴───┴───┴───┴───┘                      │   │
│   │                                                         │   │
│   │  如果队列满 → 返回 ERR__QUEUE_FULL                        │   │
│   └─────────────────────────────────────────────────────────┘   │
│      │                                                          │
│      ▼                                                          │
│   返回 ERR_NO_ERROR (仅表示已进入队列，不代表发送成功！)          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
返回成功 ≠ 消息已发送到 Kafka
返回成功 = 消息已放入内存队列，等待后台发送

真正的发送结果需要通过 poll() + 回调来获取
```

#### <2>poll — 处理回调
```cpp
int poll(int timeout_ms);  // 返回处理的事件数量
```
##### 1）功能
- 处理内部事件，**触发回调函数**
```cpp
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   后台线程发送完成后，将结果放入【结果队列】                        │
│                                                                 │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │              结果队列 (Delivery Report Queue)            │   │
│   │                                                         │   │
│   │  ┌────────┬────────┬────────┬────────┐                  │   │
│   │  │ 成功✓  │ 失败✗  │ 成功✓  │ 成功✓  │  ← 等待处理       │   │
│   │  │offset:5│timeout │offset:6│offset:7│                  │   │
│   │  └────────┴────────┴────────┴────────┘                  │   │
│   └─────────────────────────────────────────────────────────┘   │
│                          │                                      │
│                          │ poll(timeout_ms)                     │
...
如果不调用：
1. 回调永远不会触发 → 你不知道消息是否成功
2. 结果队列堆积 → 内存持续增长
3. 内部缓冲区可能被占满 → produce() 返回 QUEUE_FULL
```

#### <3>flush — 等待完成
```cpp
RdKafka::ErrorCode flush(int timeout_ms);
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   调用 flush(10000)  // 最多等待 10 秒                           │
│           │                                                     │
│           ▼                                                     │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │                    循环检查                               │   │
│   │                                                         │   │
│   │   while (outq_len() > 0 && 未超时) {                     │   │
│   │       poll(100);  // 内部自动调用 poll                    │   │
│   │       等待后台线程发送...                                 │   │
│   │   }                                                     │   │
│   │                                                         │   │
│   └─────────────────────────────────────────────────────────┘   │
...
│   │                                                         │   │
│   │   if (outq_len() == 0) {                                │   │
│   │       return ERR_NO_ERROR;  // 全部发送完成              │   │
│   │   } else {                                              │   │
│   │       return ERR__TIMED_OUT;  // 超时，仍有消息未发送     │   │
│   │   }                                                     │   │
│   └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
##### 2）outq_len() 的含义
```cpp
int outq_len = producer->outq_len();
```
![](https://cdn.jsdelivr.net/gh/xianyubuxian-TXY/hexo-images/images/20260206144906758.png)
```cpp
⚠️ outq_len() 包含两部分：
   - 等待发送的消息
   - 已发送但回调未处理的消息

所以 flush() 内部会同时等待：发送完成 + 回调处理完成
```
...
│    ┌─────────────────────────────────────────────────────────┐│                     │
│    │                                                         ││                     │
│    │  [消息在队列等待] → [批量打包] → [发送到Broker] → [收到ACK]│                     │
│    │        │              │              │              │   ││                     │
│    │     t=0ms          t=5ms          t=30ms         t=80ms ││                     │
│    │                                                         ││                     │
│    │                                          结果放入回调队列─┘│                     │
│    └─────────────────────────────────────────────────────────┘                      │
│                                                                                     │
...
│  关键时间点：                                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────────┐   │
│  │ t=0ms   : produce() 返回成功，消息进入队列                                    │   │
│  │ t=5ms   : 后台线程开始打包（等待 linger.ms 或达到 batch.size）                 │   │
│  │ t=30ms  : 消息发送到 Kafka Broker                                            │   │
│  │ t=80ms  : 收到 Broker ACK，结果进入回调队列                                   │   │
│  │ t=100ms : poll() 触发 dr_cb，你知道消息【真正】发送成功了                      │   │
│  └─────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
ProduceContext ctx;
producer->produce(topic, partition, flags, payload, len, 
                  key, key_len, timestamp, &ctx);  // 传入指针

// 在 dr_cb 中：
void dr_cb(RdKafka::Message& msg) override {
    auto* ctx = static_cast<ProduceContext*>(msg.msg_opaque());
    if (ctx) {
        ctx->success = (msg.err() == RdKafka::ERR_NO_ERROR);
        ctx->delivered = true;
    }
}
```
...
##### 模式 3：平衡模式（独立 poll 线程）
- 适合：生产环境，兼顾吞吐和响应
```cpp
class ProducerWithPollThread {
public:
    ProducerWithPollThread() {
        // 启动独立的 poll 线程
...
│   主线程                         Poll 线程                   │
│   ┌─────────────────┐           ┌─────────────────────┐     │
│   │ produce()       │           │ while (!shutdown) { │     │
│   │ produce()       │           │     poll(100);      │     │
│   │ produce()       │           │ }                   │     │
│   │ ...             │           └─────────────────────┘     │
│   └─────────────────┘                                       │
│                                                             │
│   优点：produce() 完全不阻塞，回调处理不影响主线程             │
└─────────────────────────────────────────────────────────────┘
```

#### <6>常见问题图解
##### 问题 1：只 produce 不 poll
```cpp
┌─────────────────────────────────────────────────────────────────┐
│                    ❌ 错误用法：只 produce 不 poll               │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   发送队列 (outq):                                               │
│   ┌──────────────────────────────────────────────────────┐     │
│   │  增长原因：produce 速度 > 网络发送速度                  │     │
│   │  与 poll() 无关，后台线程会自动消费                     │     │
│   │                                                      │     │
│   │  如果队列满 → produce() 返回 ERR__QUEUE_FULL          │     │
│   └──────────────────────────────────────────────────────┘     │
│                                                                 │
│   回调队列 (dr_queue):                                          │
│   ┌──────────────────────────────────────────────────────┐     │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   producer->produce(topic, "important message");                │
│   return 0;  // 直接退出程序                                    │
│                                                                 │
│   结果：                                                        │
│   ┌──────────────────────────────────────────────────────┐     │
│   │                                                      │     │
│   │   消息还在队列中 → 程序退出 → Producer 销毁           │     │
│   │                           │                          │     │
│   │                           ▼                          │     │
│   │                     消息丢失！！！                    │     │
│   │                                                      │     │
...
public:
    ...
    *           在这种情况下请改用 topic_name()。 */
    ...
    /** @returns 生产消息时传入的 msg_opaque 指针 */
