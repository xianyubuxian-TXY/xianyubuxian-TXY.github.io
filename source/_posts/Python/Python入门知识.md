...
# ❌ 忘记使用 nonlocal 会报错（因为内部作用域不能直接修改外部作用域中的变量）
def counter_wrong():
    count = 0
    def increment():
...
#### <2>闭包的核心机制：变量状态保持
##### 1）python的内存管理机制
- Python 使用**引用计数**来管理内存
    - 当一个对象没有任何引用时，才会被垃圾回收
    - 闭包中，内部函数引用了外部变量 → 引用计数 > 0 → 不会被销毁
...
normal_function()  # 输出 10
# 此时 x 已经不存在了，无法再访问
```
- 正常情况：函数执行完 → 局部变量销毁 → 内存释放
...
│  └─────────────────────┘                                    │
│                                                             │
│  第2步：outer 执行完毕，返回 inner 函数                         │
│  ⚠ 正常情况：num1 应该被销毁                                  │
│  ✅ 但因为 inner 引用了 num1，所以 num1 被"捕获"保留！           │
│                                                             │
│  第3步：f = inner（f 现在指向 inner 函数）                      │
│  ┌─────────────────────────────┐                            │
│  │  f（inner函数）              │                            │
│  │  ┌───────────────────────┐ │                            │
│  │  │ 闭包环境（被捕获的变量）  │ │                            │
...
```
```python
print(f(5))   # inner(5) → num1 + num2 → 10 + 5 = 15
print(f(6))   # inner(6) → num1 + num2 → 10 + 6 = 16
#                          ↑
#                    num1 依然是 10，没有消失！
```
...
# 创建计数器
c1 = counter()
c2 = counter()  # 另一个独立的计数器

print(c1())  # 1  ← c1 的 count 从 0 变成 1
print(c1())  # 2  ← c1 的 count 从 1 变成 2
print(c1())  # 3  ← c1 的 count 从 2 变成 3

print(c2())  # 1  ← c2 有自己独立的 count！
print(c2())  # 2

print(c1())  # 4  ← c1 的 count 不受 c2 影响
```
- 每次调用 counter() 都创建一个新的闭包环境：
```python
c1 ─────┬──→ increment函数
        └──→ 闭包环境：count = 3（独立的）
